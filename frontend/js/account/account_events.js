...
(function(window, $) {
    function bindUIEvents() {
      ...
      $(document).on("submit", "#edit-account-form", function(e) {
        ...
        window.AccountAPI.updateAccountInfo(payload, {
          onSuccess: resp => handleResponse(resp, {
          ...
          onError: xhr => handleResponse(xhr.responseJSON||{}, { errorMessage: "Update failed." })
        });
      });
      ...
      $(document).on("submit", "#add-payment-method-form", function(e) {
        ...
        window.AccountAPI.addPaymentMethod(payload, {
          ...
          onError: xhr => handleResponse(xhr.responseJSON||{}, { errorMessage: "Could not add payment method." })
        });
      });
      ...
      $(document).on("submit", "#change-password-form", function(e) {
        ...
        window.AccountAPI.changePassword(data, {
          ...
          onError: xhr => handleResponse(xhr.responseJSON||{}, { errorMessage: "Password update failed." })
        });
      });
    ...
    $(document).ready(function() {
      ...
      window.AccountAPI.getAccountData({ onSuccess: window.AccountRender.renderAccountInfo, onError: xhr =>
        handleResponse(xhr.responseJSON||{}, { errorMessage: "Failed to load account info." })
      });
  
      window.AccountAPI.loadPaymentMethods({
        onSuccess: window.AccountRender.renderPaymentMethods,
        onError: xhr => handleResponse(xhr.responseJSON||{}, { errorMessage: "Failed to load payment methods." })
      });
  
      window.AccountAPI.loadOrders({
        onSuccess: window.AccountRender.renderOrders,
        onError: xhr => handleResponse(xhr.responseJSON||{}, { errorMessage: "Failed to load orders." })
      });
