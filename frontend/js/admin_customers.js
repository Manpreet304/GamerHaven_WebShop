...
$(function(){
  ...
  // 1) Alle Kunden laden und Tabelle rendern (nur Edit‑Button)
  function loadCustomers() {
    $.getJSON('../../backend/api/ApiAdmin.php?listCustomers')
      ...
      .fail((xhr, status, err) => console.error('loadCustomers failed', status, err));
  }
  ...
  function openCustomerModal(id) {
    ...
    if (id) {
      $.getJSON(`../../backend/api/ApiAdmin.php?getCustomer&id=${id}`)
        ...
        .fail((xhr, status, err) => console.error('getCustomer failed', status, err));
    }
  ...
  function saveCustomer() {
    ...
    .fail((xhr, status, err) => console.error('saveCustomer failed', status, err))
    .always(() => {
      bootstrap.Modal.getInstance(document.getElementById('customerModal')).hide();
    });
  }
  
  // 4) Events binden (nur Edit, kein Toggle)
  ...
  // Hilfsfunktion: Form zurücksetzen
