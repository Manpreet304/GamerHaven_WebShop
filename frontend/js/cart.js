$(document).ready(function () {
    ...
    // Tooltips (falls in future benötigt)
    ...
    $(document).on("submit", "#checkout-form", function (e) {
        ...
        $.ajax({
            ...
            success(resp) {
                if (resp.success) {
                    // 1) Rechnung sofort im neuen Tab öffnen
                    window.open(`../../backend/invoices/Invoice.php?orderId=${resp.orderId}`, "_blank");
                    // 2) Erfolg anzeigen und weiterleiten
                    showMessage("success", "Your order was successfully placed! Ihre Rechnung öffnet sich in einem neuen Tab.");
...
// Lädt Warenkorb & aktiviert/deaktiviert Checkout‑Button
function loadCart() {
